  ...
  var Matrix = Base.extend(
    {
      ...
      isIdentity: function () {
        return (
          this._a === 1 && this._b === 0 && this._c === 0 && this._d === 1 && this._tx === 0 && this._ty === 0
        );
      },
  ...
  var Project = PaperScopeItem.extend({
    ...
    _insertItem: function (index, item, _created) {
      item =
        ...
        (this._activeLayer || this._insertItem(undefined, new Layer(Item.NO_INSERT), true)).insertChild(
          index,
          item,
        );
      ...
      },
      {
        ...
        _getCachedBounds: function (matrix, options, noInternal) {
          ...
          var internal = options.internal && !noInternal,
            ...
            cacheKey =
              cacheItem && (!matrix || matrix.equals(_matrix)) && this._getBoundsCacheKey(options, internal),
            bounds = this._bounds;
    ...
    ),
    {
      ...
      _drawSelection: function (ctx, matrix, size, selectionItems, updateVersion) {
        ...
        if ((itemSelected || boundsSelected || positionSelected) && this._isUpdated(updateVersion)) {
          var layer,
            color =
              this.getSelectedColor(true) || ((layer = this.getLayer()) && layer.getSelectedColor(true)),
          ...
          classify: function (v) {
            ...
            function type(type, t1, t2) {
              ...
              return {
                type: type,
                roots:
                  t1Ok || t2Ok ? (t1Ok && t2Ok ? (t1 < t2 ? [t1, t2] : [t2, t1]) : [t1Ok ? t1 : t2]) : null,
              };
            }
      ...
      getSegment: function () {
        ...
        if (!segment) {
          ...
          if (time === 0) {
            ...
            segment =
              curve.getPartLength(0, time) < curve.getPartLength(time, 1) ? curve._segment1 : curve._segment2;
          }
          this._segment = segment;
        }
        return segment;
  ...
  var Path = PathItem.extend(
    ...
    },
    {
      ...
      split: function (index, time) {
        var curve,
          location =
            time === undefined ? index : (curve = this.getCurves()[index]) && curve.getLocationAtTime(time);
      ...
      _hitTestSelf: function (point, options, viewMatrix, strokeMatrix) {
        ...
        if (options.ends && !options.segments && !closed) {
          if (
            (res =
              checkSegmentPoints(segments[0], true) || checkSegmentPoints(segments[numSegments - 1], true))
          )
            return res;
      ...
      getPathData: function (_matrix, _precision) {
        ...
        for (var i = 0, l = children.length; i < l; i++) {
          var child = children[i],
            mx = child._matrix;
          paths.push(
            child.getPathData(_matrix && !mx.isIdentity() ? _matrix.appended(mx) : _matrix, _precision),
          );
        }
        return paths.join('');
      },
        ...
        function handleCurve(v) {
          ...
          if (po <= max(o0, o1, o2, o3) && po >= min(o0, o1, o2, o3)) {
            var a0 = v[ia + 0],
              ...
              a3 = v[ia + 6],
              monoCurves =
                paL > max(a0, a1, a2, a3) || paR < min(a0, a1, a2, a3) ? [v] : Curve.getMonoCurves(v, dir),
              res;
  ...
  new (function () {
    ...
    function importSVG(source, options, owner) {
      ...
      function onLoad(svg) {
        try {
          var node =
            typeof svg === 'object' ? svg : new self.DOMParser().parseFromString(svg.trim(), 'image/svg+xml');
          if (!node.nodeName) {
            node = null;
